...
export default function FaceRecognition({ onCapture, label = "Start Face Scan" }) {
  ...
  const [status, setStatus] = useState("idle"); // idle | loadingModels | loadingCam | scanning | captured | error
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setStatus("error");
        setMessage("Failed to load face recognition models. Ensure /public/models/ files exist.");
      }
    };
    load();
    return () => stopAll();
  ...
  const stopAll = useCallback(() => {
    ...
    // Safe null check before touching DOM ref
    if (videoRef.current) {
  ...
  const startCamera = async () => {
    if (!modelsLoaded) {
      setMessage("Models still loading, please wait...");
      return;
    }
    ...
    try {
      ...
      // videoRef is always mounted (video is always in DOM, just hidden)
      ...
      if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") {
        ...
        setMessage("Camera is already in use by another app. Close it and retry.");
      } else {
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full">
      ...
      {/* ── Video element: ALWAYS in DOM so ref is never null ── */}
      <div className={`w-full flex flex-col items-center gap-3 ${isScanning ? "block" : "hidden"}`}>
        ...
        <div className={`text-sm font-medium px-4 py-2 rounded-xl w-full text-center ${
          faceBox ? "bg-green-50 text-green-700" : "bg-yellow-50 text-yellow-700"
        }`}>
          {message}
        </div>

        <div className="flex gap-3 w-full">
          <button type="button" onClick={capture} disabled={!faceBox}
          ...
          <button type="button" onClick={cancel}
            className="bg-gray-400 text-white px-5 py-2.5 rounded-xl hover:bg-gray-500 transition font-medium text-sm">
            Cancel
          </button>
        </div>
      </div>
