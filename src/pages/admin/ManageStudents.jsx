...
import { 
  Search, Filter, Plus, Edit2, X, 
  Phone, Calendar, User, Banknote, Hash, ArrowRight, CreditCard, MapPin
} from "lucide-react";
...
const ManageStudents = () => {
  ...
  const calculateAmount = () => {
    const seatPrice = formData.seatType === "AC" ? 800 : 600;
  ...
  const toggleStatus = async (student, e) => {
    ...
    try {
      await api.put(
        `/students/update/${student.studentId}`, // <-- use studentId from DB
        { status: newStatus }, // only send the updated field
        { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
      );
  ...
  const handleSubmit = async () => {
    try {
      const { name, email, course, year, seat, durationMonths } = formData;
      if (!name || !email || !course || !year || !seat || !durationMonths) {
        return toast.error("Please fill all required fields: Name, Email, Course, Year, Seat, Duration!");
      }
      ...
      const amountPaid = calculateAmount();
      const payload = { ...formData, expiry: expiryDate, status: "Active", amountPaid };

      if (isEdit) {
        await api.put(`/students/update/${formData.studentId}`, payload, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
        });
        alert("Student updated successfully!");
        toast.success("Student updated successfully!");
        setStudents(prev => prev.map(s => s.studentId === formData.studentId ? { ...formData, expiry: expiryDate } : s));
      } else {
        const res = await api.post("/students/create", payload, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
        });
        alert(`Student created successfully! Student ID: ${res.data.studentId}`);
        toast.success(`Student created! ID: ${res.data.studentId}`);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-50">
        <div>
          ...
          <p className="text-slate-500 font-medium">Manage admissions, seats, and fees</p>
        </div>
        <button onClick={openAddModal} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-2xl font-black flex items-center gap-2 transition-all shadow-lg shadow-indigo-100">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-50">
            {filteredStudents.map(s => (
              <tr key={s._id} onClick={() => navigate(`/admin/students/${s.studentId}`)}
...
// ---------- Student Modal ----------
const StudentModal = ({ formData, setFormData, handleChange, handleSubmit, setShowModal, isEdit, loading }) => {
  const calculateExpiry = () => {
    if (!formData.durationMonths) return 'Select duration';
    const expiry = new Date();
    expiry.setMonth(expiry.getMonth() + parseInt(formData.durationMonths));
      ...
      <div className="bg-white rounded-[3rem] p-8 md:p-12 w-full max-w-3xl shadow-2xl animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto no-scrollbar">
        <div className="flex justify-between items-center mb-10">
          <div>
            ...
            {!isEdit && <p className="text-slate-400 font-bold text-sm">Student ID will be auto-generated</p>}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <InputGroup icon={<User size={18}/>} label="Full Name" name="name" value={formData.name} onChange={handleChange} placeholder="John Doe" required />
          <InputGroup icon={<User size={18}/>} label="Email" name="email" type="email" value={formData.email} onChange={handleChange} placeholder="john@example.com" required />
          <InputGroup icon={<Phone size={18}/>} label="Phone Number" name="phone" value={formData.phone} onChange={handleChange} placeholder="9876543210" />
          <InputGroup icon={<CreditCard size={18}/>} label="Course" name="course" value={formData.course} onChange={handleChange} placeholder="B.Tech" required />
          <InputGroup icon={<Calendar size={18}/>} label="Year" name="year" value={formData.year} onChange={handleChange} placeholder="2026" required />
          <InputGroup icon={<Hash size={18}/>} label="Seat Number" name="seat" value={formData.seat} onChange={handleChange} placeholder="A-12" required />
          <InputGroup icon={<CreditCard size={18}/>} label="Aadhar (Last 4)" name="aadharLast4" value={formData.aadharLast4} onChange={handleChange} placeholder="1234" maxLength="4" />
          
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Seat Type</label>
            <select name="seatType" value={formData.seatType} onChange={handleChange} className="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-indigo-600 transition-all">
          ...
          <div className="flex flex-col gap-2">
            ...
            <select name="durationMonths" value={formData.durationMonths} onChange={handleChange} className="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-indigo-600 transition-all">
          ...
          <div className="flex flex-col gap-2">
            ...
            <select name="paymentMethod" value={formData.paymentMethod} onChange={handleChange} className="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-indigo-600 transition-all">
              <option>Cash</option>
              <option>UPI</option>
            ...
            <div className="flex gap-2">
              {['Male', 'Female'].map(g => (
                <button key={g} type="button" onClick={() => setFormData({...formData, gender: g})} 
                  className={`flex-1 py-4 rounded-xl font-bold transition-all ${formData.gender === g ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-slate-50 text-slate-500'}`}>
                  {g}
                </button>
              ))}
            </div>
          ...
          <div className="md:col-span-2">
            <InputGroup icon={<MapPin size={18}/>} label="Address" name="address" value={formData.address} onChange={handleChange} placeholder="City, State" />
          </div>
        </div>

        {/* Expiry & Amount Preview */}
        <div className="mt-6 p-6 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-2xl border-2 border-indigo-100">
          <div className="grid grid-cols-3 gap-4">
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Seat Type</p>
              <p className="text-xl font-black text-indigo-600 mt-1">{formData.seatType}</p>
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Expiry Date</p>
              <p className="text-xl font-black text-indigo-600 mt-1">{calculateExpiry()}</p>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Amount</p>
              <p className="text-2xl font-black text-emerald-600 mt-1">₹{calculateAmount()}</p>
            </div>
          </div>
        </div>
        ...
        <button onClick={handleSubmit} disabled={loading} className="w-full mt-10 bg-indigo-600 hover:bg-indigo-700 text-white py-5 rounded-2xl font-black text-lg transition-all shadow-xl shadow-indigo-100 disabled:opacity-50 disabled:cursor-not-allowed">
          {loading ? (
            ...
            isEdit ? "Update Student Profile" : "Confirm Admission & Payment"
          )}
        </button>
      </div>
    </div>
